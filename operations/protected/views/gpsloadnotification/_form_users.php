            ...
            <fieldset class="portlet " >
                    ...
                    <table class="table uploading-status" border='0' id="table_user">
                        ...
                        <tbody id="user">
                          <?php  //$aa = Gpsalertsusers::model()->findAll(array('condition' => 'id_gps_alerts=' . (int) $_GET['id'])); ?>
							...
							$rows=Yii::app()->db->createCommand("select gau.id_gps_alerts_users,c.fullname,c.mobile,c.gps_account_id from eg_gps_alerts_users gau,eg_customer c where (c.mobile=gau.id_customer_mobile or c.mobile=gau.gps_account_id) and gau.id_gps_alerts='".(int) $_GET['id']."' and c.type!='G'")->queryAll();
                            foreach ($rows as $user):
                              ?>
                                <tr id="row_user_<?php echo $user[id_gps_alerts_users]; ?>">
                                    <td><?php echo $user[fullname]; ?></td>
                                    <td><?php echo $user[mobile]; ?>,<?php echo $user[gps_account_id]; ?></td>
                                    <td <?php echo $deleteRow;?>> <a onclick="fnDeleteUser('<?php echo $user[id_gps_alerts_users];?>')" ><i class="delete-icon-block"></i></a> </td>
                                    </tr>
                            <?php endforeach; ?>    
                        </tbody>
                        <tfoot>
                ...
                <div class="span8"> 
                        ...
                        echo $form->textFieldRow(
                                    $model['gau'], 'id_customer_mobile', array('onkeydown' => 'fnKeyDown("Gpsalertsusers_id_customer_mobile")','rel' => 'tooltip','name'=>'Gpsalertsusers[id_customer_mobile]','id'=>'Gpsalertsusers_id_customer_mobile', 'data-toggle' => "tooltip",
