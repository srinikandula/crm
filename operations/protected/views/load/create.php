...
<script src="https://maps.googleapis.com/maps/api/js?key=<?php echo Library::getGoogleMapsKey();?>&libraries=places"></script>
    ...
    var input4 = document.getElementById('Truck_01_destination_address');
...
$(function() {
var availableTags = [
<?php foreach($records['customer'] as $row){ echo $pre.'"'.$row->idprefix.','.$row->fullname.','.Library::getCustomerType($row->type).','.$row->mobile.','.$row->email.','.$row->landline.'"';$pre=",";}?>
];
function split( val ) {
return val.split( /,\s*/ );
}
