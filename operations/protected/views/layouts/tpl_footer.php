...
<?php //echo $this->uniqueid;
//echo '<pre>';print_r($this->menu);echo '</pre>';?>
<h4 class="menu_sitemap">SiteMap</h4>

<?php 
$menu=$this->menu;
foreach($menu[0] as $key=>$val){?>
<ul class="customer_footer">
  <li><b><?php echo ucfirst($key);?></b></li>
...
$(document).ready(function(){
	...
	$("#autosuggest_mobile").autocomplete({
		...
		source: '<?php echo $this->createUrl("guest/autosuggestmobile")."/page/".urlencode($this->uniqueid."##".Yii::app()->controller->action->id);?>',
		minLength: 1
	});	
});*/	
</script>
<?php //echo $this->createUrl("guest/autosuggestmobile")."/page/".urlencode($this->uniqueid."##".Yii::app()->controller->action->id);?>
</div>