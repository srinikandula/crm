            ...
            <fieldset class="portlet " >
                ...
                <div class="portlet-content" id="hide_box_line4">
                    <table>
                        ...
                        <tbody><tr>
                                <td><?php  
                        $planlist=CHtml::listData(GpsDevicePlans::model()->findAll(array('select' => 'concat(id_device_plans,"#",plan_name,"#",amount,"#",duration_in_months) as id_device_plans,concat(plan_name," , ",amount) as plan_name ,duration_in_months','condition'=>'id_franchise="'.(int)$_SESSION['id_franchise'].'"')), 'id_device_plans', 'plan_name');
                        ...
                        <tbody id="gps_plan">
                          <?php  //$aa = Gpsalertsinterested::model()->findAll(array('condition' => 'id_gps_alert='. (int) $_GET['id'] )); ?>
                            <?php $bb = $_GET['ids']['deviceID']; ?>
                              <?php $rows=Yii::app()->db_gts->createCommand("select remark,received,id,planName,planAmount,amount,creationTime,startTime,expiryTime from Accountdeviceplanhistory where deviceID='$bb' order by creationTime DESC")->queryAll();
                              ...
                              ?>
                                <tr id="row_gps_plan_<?php echo $user[id]; ?>">
									...
									<td><textarea onblur="fnUpdateDetails(this)" rows="2" cols="10" name="planremark_<?php echo $user[id]; ?>"  id="planremark_<?php echo $user[id]; ?>" ><?php echo $user[remark]; ?></textarea></td>
...
function fnDeleteGpsPlan(id) {
    <?php if($_SESSION['id_admin_role']==1 || $_SESSION['id_admin_role']==16){?>
...
var sp=field.split('_');
var id=sp['1'];
//alert(id);
$.ajax({url: "<?php echo $this->createUrl('gpsdevices/updateFields');?>",
		type:'POST',
