        ...
        <fieldset class="portlet">
			...
			<div class="row">
                ...
                <div class="span5">
					...
					$actCond=(int)$_SESSION['id_franchise']==1?'isActive=1 order by accountID asc':'retainedEventAge="'.(int)$_SESSION['id_franchise'].'" and isActive=1 order by accountID asc';
                    $list = CHtml::listData(GpsAccount::model()->findAll(array('condition'=>$actCond)), 'accountID', 'accountID'); 
			...
			echo $form->dropDownListRow($model, 'truckTypeId', $list,array('prompt'=>'Select a Truck')); ?>
                ...
                <div class="span5"> 
				<?php	echo $form->dropDownListRow($model, 'vehicleType', array('TK'=>'Truck','CR'=>'Car','TR'=>'Transporter','NTK'=>'Non Truck'),array('prompt'=>'Select'));?>	
				</div>
				<div class="span5">
                    <?php 
					...
					/*$list = CHtml::listData(Admin::model()->findAll(array('select'=>'concat(first_name," ",last_name) as first_name,concat(id_admin,"@@",first_name," ",last_name) as id_admin','condition'=>'id_franchise="'.(int)$_SESSION['id_franchise'].'" and status=1','order'=>'first_name asc')), 'id_admin', 'first_name');*/
					
					if($_SESSION['franchise']!='easygaadi'){
						$list = CHtml::listData(Admin::model()->findAll(array('select'=>'concat(first_name," ",last_name) as first_name,concat(id_admin,"@@",first_name," ",last_name) as id_admin','condition'=>'id_franchise="'.(int)$_SESSION['id_franchise'].'" and status=1','order'=>'first_name asc')), 'id_admin', 'first_name');
					}else{
						$criteria=new CDbCriteria;
						$criteria->select='concat(t.first_name," ",t.last_name,"-",f.account) as first_name,concat(t.id_admin,"@@",t.first_name," ",t.last_name) as id_admin';
						$criteria->join="left join {{franchise}} f on t.id_franchise=f.id_franchise";
						$criteria->condition='t.status=1 order by f.account asc';
						$list = CHtml::listData(Admin::model()->findAll($criteria), 'id_admin', 'first_name');
					}
