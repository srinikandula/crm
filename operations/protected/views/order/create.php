...
?>
<?php echo $form->errorSummary($model['o']); 
?>
<div class="row-fluid">
    ...
    <?php $this->renderPartial('_create_form', array('form'=>$form,'model'=>$model,'records'=>$records));?>
<?php
$this->endWidget();
unset($form);
...
<!-- <script src="http://maps.googleapis.com/maps/api/js?sensor=false&amp;libraries=places" type="text/javascript"></script> -->
    <script type='text/javascript'>
        var input3 = document.getElementById('Order_source_address');
    var autocomplete = new google.maps.places.Autocomplete(input3);
    
    var input4 = document.getElementById('Order_destination_address');
...
$(function() {
var availableTags = [
<?php foreach($records['customer'] as $row){ echo $pre.'"'.$row->fullname.','.Library::getCustomerType($row->type).','.$row->mobile.','.$row->idprefix.'"';$pre=",";}?>
];
function split( val ) {
return val.split( /,\s*/ );
}
