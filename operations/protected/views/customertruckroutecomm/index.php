...
1.customer based,2.truck based,3.Customer truck route based,4.Route based,5.Point based,6.default global commission.if none of the condition is matched then global commission will be applied to the price.</div><div>Here, the given commission will be applied to selected truck for the given route</div><?php 
$form = $this->beginWidget(
    'bootstrap.widgets.TbActiveForm',
    array(
        'id' => 'horizontalForm',
        'type' => 'horizontal',
        'action'=>$this->createUrl('customertruckroutecomm/update'),
...
<?php
$this->renderPartial('_person_truck_route', array('form'=>$form,'model'=>$model));
...
var availableTags_PersonTruck = [
<?foreach($records['customer'] as $row){ echo $pre.'"'.$row->fullname.','.Library::getCustomerType($row->type).','.$row->mobile.','.$row->truck_reg_no.'"';$pre=",";}?>
];
...
if(type=='PersonTruck'){
var availableTags=availableTags_PersonTruck;
} else if(type=='Source'){
