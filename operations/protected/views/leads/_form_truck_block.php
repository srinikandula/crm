                ...
                foreach ($model['t'] as $tObj) {
                    ...
                    <tbody id="row1-<?php echo $row_truck; ?>">
                        <tr><td><input type="text" name="Truck[<?php echo $row_truck; ?>1][truck_reg_no]" value="<?php echo $tObj->truck_reg_no; ?>"></td>
							...
							<td>
							<?php echo CHtml::dropdownlist('Truck[' . $row_truck . '1][mfg]', $tObj->mfg, Library::getManufacturers(),array("prompt"=>"select")); ?>
							</td>
...
$truck_type = "";
//foreach (Trucktype::model()->findAll() as $ttypeRow) {
foreach ($model['truckTypes'] as $ttypeRow) {
    $truck_type.='<option value="' . $ttypeRow->id_truck_type . '" >' . $ttypeRow->title . ' ' . $ttypeRow->tonnes . '</option>';
}

$mfg = '<option value="">Select</option>';
foreach (Library::getManufacturers() as $k=>$v) {
    $mfg.='<option value="' . $k . '" >' . $v . '</option>';
}
...
<script type="text/javascript">
    ...
    function addTruck()
    {
        ...
        row += '<td><select name="Truck[' + row_no + '1][mfg]"><?php echo $mfg; ?></select></td>';
		/*row += '<td><input id="mfg" type="text" name="Truck[' + row_no + '1][mfg]"></td>';
