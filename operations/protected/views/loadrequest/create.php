...
<script src="https://maps.googleapis.com/maps/api/js?key=<?php echo Library::getGoogleMapsKey();?>&libraries=places"></script>-->
    <script type='text/javascript'>
        var input3 = document.getElementById('Truck_01_source_address');
    var autocomplete = new google.maps.places.Autocomplete(input3);
    
    var input4 = document.getElementById('Truck_010_destination_address');
    var autocomplete = new google.maps.places.Autocomplete(input4);
    
    var input5 = document.getElementById('Truck_011_destination_address');
    var autocomplete = new google.maps.places.Autocomplete(input5);
    
    var input6 = document.getElementById('Truck_012_destination_address');
...
$(function() {
var availableTags = [
<?php foreach($records['customer'] as $row){ echo $pre.'"'.$row->idprefix.','.$row->fullname.','.Library::getCustomerType($row->type).','.$row->mobile.','.$row->email.'"';$pre=",";}?>
];
function split( val ) {
return val.split( /,\s*/ );
}
