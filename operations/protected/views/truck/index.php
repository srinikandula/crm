...
<div class="note_box" style="top:62px">
	...
	<div class="note">Trucks expires in 5 days.</div>
        <div class="ad5"><div class="clr_plan"></div>
	<div class="note">Plan Expired Trucks.</div></div>
        <div class="ad"><div class="clr_plan5"></div>
            <div class="note">Plan expires in 5 days.</div></div>
...
<div class="row-fluid">
    <div class="span12 top_box_margin">
        ...
        $this->widget('bootstrap.widgets.TbExtendedGridView',
            ...
            'filter' => $model,
            'rowCssClassExpression' => '( $row%2 ? $this->rowCssClass[1] : $this->rowCssClass[0] ).(($data->vehicle_insurance_expiry_date <= date("Y-m-d")?" alert":"") || ($data->fitness_certificate_expiry_date) <= date("Y-m-d")?" alert":"").((date("Y-m-d") <= $data->fitness_certificate_expiry_date) && ($data->fitness_certificate_expiry_date) <=(date("Y-m-d", strtotime("+5 days")))?" notapproved":"").((date("Y-m-d") <= $data->vehicle_insurance_expiry_date) && ($data->vehicle_insurance_expiry_date) <=(date("Y-m-d", strtotime("+5 days")))?" notapproved":"").(($data->date_end) < date("Y-m-d")?" plan_exp":"").((date("Y-m-d") <= $data->date_end) && ($data->date_end) <=(date("Y-m-d", strtotime("+5 days")))?" plan_exp5":"") ',        
            //'rowCssClassExpression' => '( $row%2 ? $this->rowCssClass[1] : $this->rowCssClass[0] ). (($data->date_end) < date("Y-m-d")?" notapproved":"")',
                'bulkActions' => array(
                'actionButtons' => array(
                ),
            ...
            'columns' => array(
                ...
                array('name' => 'id_truck_attachment_policy', 'header' => 'Plan',
				
                    'filter' => CHtml::dropDownList('Truck[id_truck_attachment_policy]',
                            $_GET['Truck']['id_truck_attachment_policy'],
                            array('' => 'All','1' => 'Per Transaction (Default)', '2' => 'Per Km With Min Guarante', '3' => 'Flat Payment With Diesel Exp',)
                    ), 'value' => array($this,'grid'),'type'=>'raw'),
                //array('name' => 'date_start', 'header' => 'Date start', 'value' => '$data->date_start'),
                array('name' => 'date_end', 'header' => 'Plan Exp Date', 'value' => '$data->date_end'),
