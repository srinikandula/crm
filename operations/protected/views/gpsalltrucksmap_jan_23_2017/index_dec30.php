...
FROM Account as a
JOIN Device as d
ON a.accountID = d.accountID AND (d.deviceID like '$curr_truck'  OR d.accountID like '$curr_truck') Or (d.truckTypeId = '$dd') and d.vehicleType='TK';")->queryAll();//queryAll()
}
?>
<?php
//$tr = array();
$trucks=Trucktype::model()->findAll(array('select' => 'title,id_truck_type', 'condition' => 'status=1'));
   //for($i=0;$i<count($trucks);$i++){
   //echo '<pre>';print_r($trucks[$i]['id_truck_type']);
       //$tr = $trucks[$i]['title'];
   //}exit;
?>
