...
class Admin extends CActiveRecord
{
	...
	public function rules()
	{
		return array(
			...
			array('admin,admin_role,id_admin,id_franchise, id_admin_role, password, first_name, last_name,admin_role, phone, email, date_created, present_visit_date, last_visit_date, date_modified, status,city,state', 'safe'),
		);
	}
	...
	{
		//exit($password." password & this password ".$this->password." ".CPasswordHelper::hashPassword($password));
    ...
    public function search()
	{
		...
		/*if($_SESSION['id_admin_role']==1){ //other users should view only there details
			$criteria->condition='t.id_franchise='.(int)$_SESSION['id_franchise'];
		}else */if($_SESSION['id_admin_role']==2){
			$criteria->condition='t.id_franchise='.(int)$_SESSION['id_franchise'];
		}else if($_SESSION['id_admin_role']!=1){ //if just user then he can see his only
			$criteria->condition='id_admin='.Yii::app()->user->id;
		}
	...
	public function getLeastAssigmentForOrders() {
        $row = Yii::app()->db->CreateCommand("select a.id_admin,(select count(*) from {{order}} o where o.id_admin_assigned=a.id_admin and date(o.date_ordered)>'" . date('Y-m-d', strtotime("-2 days")) . "') as rows from {{admin}} a where a.status=1 and  a.id_admin_role=4 order by rows asc")->queryRow(); //10=Outbound Calling Team
        return $row;
    }
	
	public function getAdminRoleDetails(){
		$rows=Yii::app()->db->createCommand("select a.*,ar.role from {{admin}} a,{{admin_role}} ar where a.id_admin_role=ar.id_admin_role and a.status=1")->queryAll();
		$data=array();
		foreach($rows as $row){
			$data[$row['id_admin']]=$row;
		}
		return $data;
	}

	public function assignToInboundUser(){
	...
	public function getFieldTeamDeviceID()
	{
		$cache=Yii::app()->cache;
		$zones=$cache->get('getFieldTeamDeviceID');
		//if($zones===false)
		if(1)
		{
			//echo "inside";
			$rows=Admin::model()->findAll(array("select"=>"distinct deviceid","condition"=>"deviceid!='' and status=1"));
			$zones=array();
			foreach($rows as $row){
				$zones[]=$row->deviceid;
			}
			
			$cache->set('getFieldTeamDeviceID',$zones , 10);
		}else{
			//echo "outside";
		}
		return $zones;
