...
class Customer extends CActiveRecord
{
	...
	public function searchCustomerTruck()
	{
        ...
        $criteria->select="(select group_concat(tr.truck_reg_no) from {{truck}} tr where tr.id_customer=t.id_customer) as truck_reg_no,t.*,(select count(*) from {{truck}} tk where tk.id_customer=t.id_customer) as no_of_trucks";
	...
	public function searchCustomerLead()
	{
		...
		if(!Yii::app()->getController('Controller')->listAllPerm){//exit('in');
			...
			$criteria->join='inner join {{customer_lead}} cl on cl.id_customer=t.id_customer'.$str.$strFltr;
		}
		//$criteria->condition='type="T"';
		//$cObj=new Controller;
