...
class Gpstrucklocation extends CActiveRecord
{
	...
	public function rules()
	{
		return array(
			array('mobile,accountid,truck_reg_no', 'required'),
                    array('address,date_available,title', 'safe'),
    ...
    public function search()
	{
        ...
        //$criteria->select="t.*,(select fullname from {{customer}} c where t.accountid= c.gps_account_id) as name,(select mobile from {{customer}} c where t.accountid= c.gps_account_id) as mobile_no,(select tt.title from {{truck_type}} tt where tt.id_truck_type=(select tr.id_truck_type from {{truck}} tr where t.truck_reg_no= tr.truck_reg_no)) as truck_type";
