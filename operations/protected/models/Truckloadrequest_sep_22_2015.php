...
class Truckloadrequest extends CActiveRecord
{
	...
	public function rules()
	{
		return array(
			...
			array('truck_type,date_created,mobile,truck_reg_no,fullname,date_available,status,make,price,source_state,source_city,source_address,title,add_info,source_lat,source_lng','safe'),
			);
	}
	...
	public function searchLoad()
	{
		...
		$criteria=new CDbCriteria;
		$criteria->select="(select group_concat(tlrd.destination_city) from {{truck_load_request_destinations}} tlrd where tlrd.id_truck_load_request=t.id_truck_load_request) as destinations, c.type,c.fullname,c.mobile,t.*,tt.title as truck_type,gt.title as goods_type";
