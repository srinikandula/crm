...
class Admin extends CActiveRecord
{
	...
	public function getLeastAssigmentId() {
        $row = Yii::app()->db->CreateCommand("select a.id_admin,(select count(*) from {{customer}} c,{{customer_lead}} cl where c.id_customer=cl.id_customer and c.islead=1 and cl.id_admin_assigned=a.id_admin and c.date_created='" . date('Y-m-d', strtotime("-1 days")) . "') as rows from {{admin}} a where a.status=1 and a.id_admin_role=10 order by rows asc")->queryRow(); //10=Outbound Calling Team
        return $row;
    }

	public function getLeastAssigmentIdSearch() {
        $row = Yii::app()->db->CreateCommand("select a.id_admin,(select count(*) from {{load_truck_request}} ltr where ltr.id_admin_assigned=a.id_admin and ltr.date_created='" . date('Y-m-d', strtotime("-1 days")) . "') as rows from {{admin}} a where a.status=1 and  a.id_admin_role=10 order by rows asc")->queryRow(); //10=Outbound Calling Team
        return $row;
    }
}
