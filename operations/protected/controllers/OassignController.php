...
class OassignController extends Controller
{   
        public $admins;
        public $adminOptions;
        public function actionUpdate($id)
	{
            ...
            if (Yii::app()->request->getIsAjaxRequest() && $aid && $id ) {
                Order::model()->updateAll(array('id_admin_assigned'=>$aid),'id_order='.$id);
                $json['status']=1;
            }
	...
	public function actionIndex()
	{
            ...
            $model=new Order('Assignment');
            $model->unsetAttributes();  // clear any default values
            if(isset($_GET['Order']))
                    $model->attributes=$_GET['Order'];
            //exit;
            $this->render('index',array('model'=>$model,'dataSet'=>$model->Assignment()));
	}
        ...
        protected function grid($data, $row, $dataColumn) {
            switch ($dataColumn->name) {
                ...
                case 'id_admin_assigned';
                    //echo '<pre>';print_r($this->adminOptions);exit;
                    $select='<select name="select" onchange="fnchange(this,'.$data['id_order'].')">';
