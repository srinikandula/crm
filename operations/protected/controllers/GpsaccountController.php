...
class GpsaccountController extends Controller
{
	...
	public function actiongetDeviceDetails(){
        ...
        $flag=0;
        if ($accountid!="") {
            $device=GpsDevice::model()->findAll(array("select"=>"vehicleType,deviceID,installedBy,vehicleModel,truckTypeId,lastValidLatitude,lastValidLongitude,lastValidSpeedKPH,lastUpdateTime,lastGPSTimestamp,imeiNumber,simPhoneNumber,creationTime","condition"=>"accountID='".$accountid."'"));
	...
	public function actionCreate()
	{
		...
		if(Yii::app()->request->isPostRequest)
		{
			...
			$model['ga']->attributes=$_POST['GpsAccount'];
						...
						$model['ga']->stopAlertTime = 180;
                        $model['ga']->retainedEventAge=$_SESSION['id_franchise'];
		...
		//echo '<pre>';print_r($data);exit;		
			if($data['accountID']!="" && $data['password']!="" && $data['contactPhone']!="" && $data['contactName']!=""){
					$findCObj=Customer::model()->find('gps_account_id like "'.$data['accountID'].'" or mobile="'.$data[contactPhone].'" and type!="G"');
					...
					$custObj->islead=0;
					$custObj->id_franchise=$_SESSION['id_franchise'];
					...
					//echo $findCObj->id_customer.'<pre>';print_r(array('mobile'=>$data[contactPhone],'idprefix'=>$idprefix,'password'=>$encyPwd,'fullname'=>$data['contactName'],'islead'=>0,'type'=>$data['customer_type'],'gps_account_id'=>Library::trimToLower($data['accountID']),'status'=>$data['isActive'],'approved'=>$data['isActive']));
					//exit("in else");
					$idprefix=Library::getIdPrefix(array('id'=>$findCObj->id_customer,'type'=>$findCObj->type));
					//echo $findCObj->id_customer.'<pre>';print_r(array('mobile'=>$data[contactPhone],'idprefix'=>$idprefix,'password'=>$encyPwd,'fullname'=>$data['contactName'],'islead'=>0,'type'=>$data['customer_type'],'gps_account_id'=>Library::trimToLower($data['accountID']),'status'=>$data['isActive'],'approved'=>$data['isActive']));exit;
					...
					Customer::model()->updateAll(array('mobile'=>$data[contactPhone],'idprefix'=>$idprefix,'password'=>$encyPwd,'fullname'=>$data['contactName'],'islead'=>0,'type'=>$data['customer_type'],'gps_account_id'=>Library::trimToLower($data['accountID']),'status'=>$data['isActive'],'approved'=>$data['isActive']),'id_customer="'.$findCObj->id_customer.'"');
        ...
        public function createCustomerTemp($data){
            //echo '<pre>';print_r($data);exit;		
            if($data['accountID']!="" && $data['password']!="" && $data['contactPhone']!="" && $data['contactName']!=""){
	...
	{       
        ...
        if(isset($_POST['GpsAccount']))
		{
			...
			{
                            $model['ga']->save(false);
							GpsDevice::model()->updateAll(array("isActive"=>(int)$_POST['GpsAccount']['isActive']),'accountID="'.$model['ga']->accountID.'"');
							$this->addCOptDest(Library::trimToLower($_POST['GpsAccount']['accountID']));
