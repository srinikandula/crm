...
class GPSapiV1Controller extends Controller {
    ...
    /*public function actionGetAlerts() { //need to upload image
        ...
        if (1) {
            ...
            $rows[1] = Yii::app()->db->CreateCommand("select ga.* from eg_gps_alerts ga,eg_gps_alerts_users gau where ga.id_gps_alerts=gau.id_gps_alerts and (gau.id_customer='".$uid."') and ga.sendtoall=0 and date(ga.date_created)+ INTERVAL 30 DAY >=NOW() order by ga.date_created desc")->queryAll();
