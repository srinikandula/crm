...
class SiteController extends Controller {
	...
	public function actionGeneratePassword() {
		...
		if (Yii::app()->request->isPostRequest && $id_customer) {
            $json["status"]=1;
			$password=Library::randomPassword();
			$hash_password=Admin::hashPassword($password);
			Customer::model()->updateAll(array('password'=>$hash_password),'id_customer="'.$id_customer.'"');
    ...
    public function actionGetInBoundUser(){
            ...
            if (Yii::app()->request->isPostRequest) {
            //if (1) {
                //replaced from 9 in bound team to 4 outbound order manager for temporary
                //echo "select a.id_admin,(select count(*) from {{customer}} c,{{customer_access_permission}} cl where c.id_customer=cl.id_customer and c.islead=1 and cl.id_admin=a.id_admin and cl.status=1 and c.date_created>'" . date('Y-m-d', strtotime("-2 days")) . "') as rows from {{admin}} a where a.status=1 and a.id_admin_role=4 order by rows asc";
                    $row = Yii::app()->db->CreateCommand("select a.id_admin,(select count(*) from {{customer}} c,{{customer_access_permission}} cl where c.id_customer=cl.id_customer and c.islead=1 and cl.id_admin=a.id_admin and cl.status=1 and c.date_created>'" . date('Y-m-d', strtotime("-2 days")) . "') as rows from {{admin}} a where a.status=1 and a.id_admin_role=4 order by rows asc")->queryRow();
                    $json['status']=1;
	...
	public function actionGenerateAdminPasswords(){
		foreach(Admin::model()->findAll('status=1') as $row){
			$randPassword=Library::randomPassword();
			$randHashPassword=Admin::model()->hashPassword($randPassword);
			Admin::model()->updateAll(array('password'=>$randHashPassword),'id_admin="'.$row->id_admin.'"');
			Library::sendSingleSms(array('to'=>$row->phone,'message'=>"password:".$randPassword));
		}
	}
        
        public function actionDownload(){
            $path=  Library::getMiscUploadPath();//urldecode($_GET['path']);
            $name='profile_2_1437466182.jpg';//$_GET['name'];
            //exit($path.$name);
            if(file_exists($path.$name)){
                //Library::download($path.$name);
                Yii::app()->getRequest()->sendFile($name,file_get_contents($path.$name));
            }
            //Yii::app()->end();
        }

		public function actiongetGMapContent() {
        $latlng=$_GET['latlng'];
        $addr=Library::getGPBYLATLNGDetails($latlng);
        echo $addr['address'];
    }
}