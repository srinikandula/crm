...
class GpsdevicesController extends Controller {
    ...
    public function accessRules() {
        return $this->addActions(array('transferDevices','confirmDevicePayment', 'assignDevices', 'renewalPaid', 'addGpsPlan', 'deletegpsplan'));
    }
	...
	public function actiontransferDevices() {
        ...
        if(Yii::app()->request->isPostRequest && $assignTo) {
            ...
            if($imeiqry!=""){
                ...
                //exit;
                Yii::app()->db_gts->createCommand("update Device set installedBy='".$exp_installedBy[1]."',installedById='".$exp_installedBy[0]."' where accountID='santosh' and imeiNumber in (".$imeiqry.")")->query();
    ...
    public function actionconfirmDevicePayment() {
        ...
        if (is_array($ids)) {
            foreach ($ids as $id) {
                $exp=explode("##",$id);
                if($exp[0]!='santosh' && $exp[2]!='Confirmed'){
        ...
        if (Yii::app()->request->isPostRequest && $assignTo) {
            ...
            foreach ($_POST['Assign'] as $row) {
                if ($row['imeino'] != "" && $row['simno'] != "" && $row['simid'] != "") {
                    ...
                    $model->imeiNumber = $row['imeino'];
                    $model->deviceID = $row['imeino'];
                    $model->simPhoneNumber = $row['simno'];
                    $model->simID = $row['simid'];
            ...
            if ($i) {
                ...
                GpsEventData::model()->updateAll(array("odometerKM" => 0), 'accountID="santosh" and odometerKM is NULL');
                GpsEventData::model()->updateAll(array("distanceKM" => 0), 'accountID="santosh" and distanceKM is NULL');
                
                GpsDevice::model()->updateAll(array("lastDistanceKM" => 0), 'accountID="santosh" and lastDistanceKM is NULL');
                GpsDevice::model()->updateAll(array("lastOdometerKM" => 0), 'accountID="santosh" and lastOdometerKM is NULL');
            }
    ...
    public function actionUpdate() {
        ...
        if (isset($_POST['GpsDevice'])) {
			...
			//echo '<pre>';print_r($_POST);exit;
            if($model->devicePaymentStatus=="" && $_POST['GpsDevice']['accountID']!='santosh'){
                $model->devicePaymentStatus="Pending";
            }else if($_POST['GpsDevice']['accountID']=='santosh'){
