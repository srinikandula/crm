...
class CagentController extends Controller {
public $getSmsEmailStatus;
    public function accessRules() {
        return $this->addActions(array('Approve','Export','Download'));
    }
    
    public function actionDownload(){
        $expObj=new Export();
        $return=$expObj->download();
        exit;
    }
    ...
    public function actionExport(){
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            if ($fileType=='xlsx' && $id && (isset( $_FILES['import'] )) && (is_uploaded_file($_FILES['import']['tmp_name']))) {
                //exit("inside ".$id);    
                $file = $_FILES['import']['tmp_name'];
                $expObj=new Export();
                $return=$expObj->upload($file);        
                $this->redirect($this->createUrl('cagent/export',array('id'=>(int)$_GET['id'],'return'=>$return)));    
                //$this->refresh();
            }else{
                $error="Invalid fileformat/user";
            }
        }
        $this->renderPartial('_export',array('model'=>Customer::model()->findByPK((int)$_GET['id']),'return'=>(int)$_GET['return'],'error'=>$error));
    }
    ...
    public function actionApprove() {
        ...
        if ($approved) {
            Yii::app()->user->setFlash('success', 'Selected Commission Agent approved successfully!!');
        }
        $this->redirect('index');
    }
...
public function addCOptDest($id)
    ...
    public function actionIndex() {
        ...
        $model = new Customer('searchCommissionAgent');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['Customer']))
            $model->attributes = $_GET['Customer'];

        $this->render('index', array(
            'model' => $model,'dataSet'=>$model->searchCommissionAgent()
        ));
    }
    ...
    protected function grid($data, $row, $dataColumn) {
        switch ($dataColumn->name) {
            case 'no_of_loads':
                ...
                //$return=" <a href='#'><i class='icon-close'></i></a>";
                break;
            ...
            case 'id_customer':
                $return='<a href="#" onclick="fnupload('.$data->id_customer.');">Click here</a>';
                break;
			case 'fullname':
