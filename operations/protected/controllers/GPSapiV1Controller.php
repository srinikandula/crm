...
class GPSapiV1Controller extends Controller {
    ...
    /*public function actionGetAlerts() { //need to upload image
	    ...
	    public function actionGetAlerts() { //need to upload image
			//$this->sendPushNotification(array("message"=>"test message"));
        /*
          http://egcrm.cloudapp.net/operations/index.php/GPSapiV1/GetAlerts
          method=post
          param=accountid
    ...
    public function actionGetOrders() { 
        ...
        */
        ...
        if (1) {
            $json['status'] = 1;
            $trRows=Yii::app()->db->createCommand("select truck_reg_no,truck_type,goods_type,order_status_name,id_order,source_city,destination_city,DATE_FORMAT( date_ordered,  '%d-%m-%Y %h:%i %p' )  as date_ordered from eg_order where (id_customer='".$uid."' or id_customer_ordered='".$uid."') order by date_ordered desc limit ".$this->limit." offset ".$offset)->queryAll();
...
DAY ) as unx_date_ordered_3  from eg_order where id_order='".$oid."'")->queryRow();
					...
					if($row['speedKPH']){
						...
						$points[$loop]['speedValue']=ceil($row['speedKPH']);
                                            ...
                                            if($prevTimestamp!="")
                                            {
												...
												if($prevTimestamp!=""){ $distance=$prevTimestamp+360<$row['time_in_secs']?1:0; }else{$distance=0;}
 ...
 public function distance($lat1, $lon1, $lat2, $lon2, $unit) {
  ...
  if ($unit == "K") {
	...
	public function actiontracktruck1() { //need to upload image
        /*
          http://egcrm.cloudapp.net/operations/index.php/GPSapiV1/tracktruck
          method=post
		  truckregno
	...
	public function sendPushNotification($data){
        ...
        if($row!=""){
			...
			echo '<pre>';print_r(array('devices'=>$devices,'message'=>array('message'=>$data['message'],'type'=>'load')));echo '</pre>';
	...
	public function actiontracktrucklast() { //need to upload image
			/*
...
foreach($rows as $row)
{
	if($row['speedKPH']){
		...
		if($rows[$i+1]['speedKPH']){
			$stops[$stopLoop]=array("latitude"=>$row['latitude'],"longitude"=>$row['longitude'],"date_time"=>$row['date_time'],"time_in_secs"=>$startingStopTime,"stopTime"=>$row['time_in_secs']-$startingStopTime,"distance"=>0);
			$stopLoop++;
			$startStopNew=1;
		}
	}
