...
class AdminroleController extends Controller
{
        ...
        {
			$id_admin_role=(int)$_SESSION['id_franchise']==1?1:2;
            $rows=AdminPermissions::model()->findAll('status=1 and id_admin_role='.$id_admin_role);
            $modules=array();
            foreach($rows as $row)
            {
	...
	public function actionCreate()
	{
		...
		if (Yii::app()->request->isPostRequest)
		{
					...
					$model->id_franchise=(int)$_SESSION['id_franchise'];
                    if($model->validate())
                    {
                        $model->save(false);
