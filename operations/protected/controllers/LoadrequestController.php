...
class LoadrequestController extends Controller {
    ...
    public function accessRules() {
        return $this->addActions(array('Routeinfo'));
    }
    
    public function actionRouteinfo(){
        $json=array();
        if(Yii::app()->request->getIsAjaxRequest()){
            $km=Library::getGMDistanceDetails(array('source'=>$_POST['src'],'destination'=>$_POST['dest']));
            $sysPrice=  Loadtruckrequest::model()->getSystemQuotes(array('source'=>$_POST['src'],'destination'=>$_POST['dest'],'id_truck_type'=>$_POST['trucktype']));
            //echo $json;exit;
            $json['min_sys_price']=$sysPrice['min'];
            $json['avg_sys_price']=$sysPrice['avg'];
            $json['distance']=$km;
        }
    ...
    public function actionCreate() {
        ...
        if (Yii::app()->request->isPostRequest && sizeof($_POST['Truck'])) {
			...
			$custObj = Customer::model()->find('mobile="' . $xCust[3] . '"');
            ...
            foreach ($_POST['Truck'] as $data) {
                ...
                $model['ltr']->title = $_POST['Truckloadrequest']['title'];
    ...
    public function actionUpdate($id) {
        ...
        $destRow=Truckloadrequestdestinations::model()->findAll(array('select'=>'destination_address,price','condition'=>'id_truck_load_request="'.$id.'"'));
        $model['dest']=$destRow;
        //exit($model['dest']);
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            //echo '<pre>';print_r($dests);print_r($model['t']->attributes);print_r($_POST);exit;
                ...
                $this->addDestinations(array('id'=>$id,'dest'=>$_POST['Truckloadrequest']['destination_address'],'price'=>$_POST['Truckloadrequest']['price']));
  ...
  public function addDestinations($data){
        ...
        foreach($dests as $k=>$dest){
            if($dest==""){continue; }
            $lower_dest=strtolower($dest);
            if(in_array($lower_dest,array('ai','si','ei','wi','ni'))){
                $dest=array('address'=>$lower_dest,'city'=>$lower_dest,'state'=>$lower_dest);
            }else{
