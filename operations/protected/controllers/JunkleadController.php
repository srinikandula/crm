...
class JunkleadController extends Controller {

    public $customerType;
    public $approved;
    public $id_customer_access_permission;
    public function accessRules() {
        return $this->addActions(array('checkunique','Approve', 'Updatestatus', 'Updateleadassigment'));
    }
    
    public function actioncheckunique(){
        $mobile=$_POST['mobile'];
        if (Yii::app()->request->getIsAjaxRequest() && Yii::app()->request->isPostRequest) {
           $obj=Customer::model()->find(array('select'=>'fullname,mobile,type','condition'=>'type!="G"  and islead=0 and mobile="'.$mobile.'"'));
           if($obj->mobile!=""){
            $status=0;
            $msg="Mobile already exist!!".$obj->fullname." , ".$obj->type;
           }else{
            $status=1;    
           }
    ...
    public function actionUpdatestatus() {
        ...
        if (Yii::app()->request->getIsAjaxRequest() && Yii::app()->request->isPostRequest && $_POST[Customerleadstatushistory][status]!='') {
            ...
            Customer::model()->updateAll(array('date_modified'=>date('Y-m-d H:i:s')),'id_customer='.$id);
    ...
    public function approveLead($input){
        ...
        if($input['status']=='Accept Approval'){
            ...
            Customer::model()->updateAll(array('date_modified'=>date('Y-m-d H:i:s'),'status'=>1,'approved'=>1,'islead'=>0,'idprefix'=>$idprefix),'id_customer="'.$input['id_customer'].'"');
            if($obj->email!=''){
    ...
    public function actionCreate() {
        ...
        //exit("value of ".Yii::app()->controller->id);
        $model['c'] = new Customer;
        $model['tt'] = new Trucktype;
            ...
            if ($model['c']->validate()) {
               ...
               // echo '<pre>';print_r($model['cvt']);exit;
                /* $data = array('id' => '2', 'replace' => array('%customer_name%' => $_POST['Customer']['fullname']), 'mail' => array("to" => array($_POST['Customer']['email'] => $_POST['Customer']['fullname'])));
                  ...
                  Mail::send($data); */
				//echo '<pre>';print_r($_POST);exit;
                if($_POST['Customerleadstatushistory']['status']=='Request For Approval'){
                    $approved=$this->approveLead(array('id_customer'=>$id,'status'=>'Accept Approval'));
                    $sucmsg=$approved==1?"Approved":"Not Approved";
                }
                Yii::app()->user->setFlash('success', Yii::t('common', 'message_create_success').$sucmsg);
                //exit;
                $this->redirect('index');
            }
	...
	$model['leadStatuses']=Library::getLeadStatuses();
        $model['existingCustomers'] = Customer::model()->getApprovedActiveCustomers();
    ...
    public function addTrucks($data){
        ...
        foreach($_POST['Truck'] as $k=>$v){
            ...
            $ml = $_POST['Truck']['01']['mileage'];
            $tt = $_POST['Truck']['01']['id_truck_type'];
            $truck=Trucktype::model()->findAll(array('select'=>'id_truck_type, mileage','condition'=>'id_truck_type='.$tt));
            $att = $truck[0]['id_truck_type'];
            $aml = $truck[0]['mileage'];
            if($ml == 0 && $tt == $att){
            $ml = $aml;
        }
        //$_POST['Truck']['01']['mileage'] = $ml;
        //$model['Truck']['01']['mileage'] = $_POST['Truck']['01']['mileage'];
            //echo '<pre>';print_r($model['s']);exit;
            ...
            $model->attributes=$v;
            $model['mileage'] = $ml;
            //echo '<pre>';print_r($model['mileage']);exit;
    ...
    public function addDriver($id){
        ...
        foreach($_POST[Driver] as $k=>$v){
            ...
            $cdc->id_customer=$id;
            $cdc->id_driver=$id_driver;
            $cdc->save(false);
            if($v[id_driver]==""){
                $cdh=new Customerdriverhistory;
                $cdh->id_customer=$id;
                $cdh->id_driver=$id_driver;
                $cdh->save(false);
            }    
        }
        //exit;
    ...
    public function actionUpdate($id) {
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            $this->addCOptDest($id);
            $this->addTrafficManagers($id);
			$model['c']->attributes = $_POST['Customer'];
			$model['c']->date_modified = date('Y-m-d H:i:s');
            ...
            $upload = Library::fileUpload($data);
            $model['c']->tds_declaration_doc = $upload['file'];
            //exit("value of ".$model['c']->password);
            $model['c']->confirm=$model['c']->password!=""?$model['c']->password:"";
            if ($model['c']->validate()) {
        ...
        $model['ctm']  = Customertrafficmanager::model()->findAll('id_customer="' . $model['c']->id_customer . '"');
    ...
    public function addTrafficManagers($id)
    {
		
        //if ($_POST['Customer']['type'] == 'C') {
            Customertrafficmanager::model()->deleteAll('id_customer="'.$id.'"');
                foreach ($_POST['Customertrafficmanager']['traffic'] as $k => $v) {
                        if ($v['mobile'] == '' && $v['full_name'] == '') {
                                continue;
						}
                       
						$model = new Customertrafficmanager;
						$model->id_customer = $id;
						$model->full_name = $v['full_name'];
            ...
            Customeroperatingdestinations::model()->deleteAll('id_customer="'.$id.'"');
                foreach ($_POST['Customer']['operating_city'] as $k => $v) {
						...
						$model['cod']->destination_lng = $gDetails2['lng'];
                                                $model['cod']->id_truck_type = $v['id_truck_type'];
    ...
    public function actionDelete() {//($id)
        ...
        if (sizeof($arrayRowId) > 0) {
			...
			$custdocObjs = Customerdocs::model()->findAll($criteria);
			foreach ($custdocObjs as $custObj) {
                unlink(Library::getMiscUploadPath() . $custObj->file);
            }
    ...
    public function actionIndex() {
        ...
        $this->customerType = array('L'=>'MF','T'=>'TO','TR'=>'TR','C'=>'CA');
        //echo '<pre>';print_r($this->customerType);exit;
        $model = new Customer('searchCustomerJunkLead');
        ...
        $this->render('//leads/index', array(
            'model' => $model,'dataSet'=>$model->searchCustomerJunkLead()
        ));
    }

    public function loadModel($id) {
        //$model['c'] = Customer::model()->find(array("condition" => "id_customer=" . $id));
		$model['c'] = Customer::model()->find(array("select"=>"t.*,cl.smartphone_available,cl.lead_source,cl.lead_status","join" => "inner join {{customer_lead}} cl on t.id_customer=cl.id_customer and t.id_customer=" . $id));
