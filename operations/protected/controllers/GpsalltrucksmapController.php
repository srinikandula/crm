...
//  echo '<pre>';print_r($dd);exit;
    $fcond=$_SESSION['id_franchise']==1?'':'  and  a.retainedEventAge="'.$_SESSION['id_franchise'].'"';    
    $truck = $_POST['truck'];
...
///$deviceid =Yii::app()->db_gts->createCommand("select accountID from device where accountID like '%".$truck."%';")->queryAll();//queryAll()
    ...
    FROM Account as a
    JOIN Device as d
     ON a.accountID = d.accountID ".$fcond." and d.lastValidLatitude!='' and d.lastValidLongitude!='' and a.accountID!='santosh' and d.isActive=1 and d.lastGPSTimestamp>".$checkTime)->queryAll();//queryAll()
	}  
    ...
    FROM Account as a
    JOIN Device as d                        
    ON  a.accountID!='santosh' and a.accountID = d.accountID   ".$fcond."  and d.isActive=1 and d.lastGPSTimestamp>".$checkTime." and (lastValidLatitude!='' and lastValidLongitude!='') AND (((d.deviceID like '%".$myArray[0]."%'  OR d.accountID like '%".$myArray[1]."%') and (d.truckTypeId = '$truck_list')) and (d.vehicleType='TK'));")->queryAll();//queryAll()
    }
    ...
    FROM Account as a
    JOIN Device as d
     ON a.accountID!='santosh' and  a.accountID = d.accountID  ".$fcond."   and d.isActive=1 and d.lastGPSTimestamp>".$checkTime." and (lastValidLatitude!='' and lastValidLongitude!='') AND ((d.deviceID like '%".$myArray[0]."%'  OR d.accountID like '%".$myArray[1]."%') and (d.vehicleType='TK'));")->queryAll();//queryAll()
	} else  if(($truck =="")&($truck_list !="")){

    $rows1=Yii::app()->db_gts->createCommand("SELECT a.accountID, a.contactName,a.contactPhone, d.deviceID, d.lastValidLatitude, d.lastValidLongitude,d.vehicleModel,ceil(d.lastValidSpeedKPH*1.852) as speed
    FROM Account as a
    JOIN Device as d
     ON a.accountID!='santosh' and  a.accountID = d.accountID  ".$fcond."  and d.isActive=1 and d.lastGPSTimestamp>".$checkTime." and (lastValidLatitude!='' and lastValidLongitude!='') AND ((d.truckTypeId = '$truck_list') and (d.vehicleType='TK'));")->queryAll();//queryAll()
	}   
