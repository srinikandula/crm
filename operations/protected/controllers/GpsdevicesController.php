...
class GpsdevicesController extends Controller {
    ...
    public function accessRules() {
        return $this->addActions(array('getDeviceDetails','downloadDevice','transferDevices','confirmDevicePayment', 'assignDevices', 'renewalPaid', 'addGpsPlan', 'deletegpsplan','updateFields'));
    }
	...
	public function actionupdateFields() {
        ...
        if ($val!="" && $field!="" && $id!=0 &&  Yii::app()->request->getIsAjaxRequest() && Yii::app()->request->isPostRequest) {
            Accountdeviceplanhistory::model()->updateAll(array('remark'=>$val),'id="'.$id.'"');
            $json['status']=1;
        }
	...
	public function actiontransferDevices() {
        ...
        //$aCond=$_SESSION['id_franchise']==1?
        //echo '<pre>';print_r($_SESSION);exit;
        //$assignToRows = Admin::model()->findAll(array("select" => "id_admin,first_name,last_name", "condition" => "id_franchise='".$_SESSION['id_franchise']."' and status=1 order by first_name desc"));
        $criteria=new CDbCriteria;
        $criteria->select="t.id_admin,t.first_name,t.last_name,f.account";
        $criteria->join="left join {{franchise}} f on t.id_franchise=f.id_franchise";
        $criteria->condition=$_SESSION['id_franchise']==1?"(t.id_franchise=1 or t.id_admin_role=2) and t.status=1 order by t.first_name asc":"t.id_franchise='".$_SESSION['id_franchise']."' and t.status=1 order by t.first_name asc";
        $assignToRows = Admin::model()->findAll($criteria);
    ...
    public function actionassignDevices() {
        ...
        $criteria->select="t.id_admin,t.first_name,t.last_name,f.account";
        $criteria->join="left join {{franchise}} f on t.id_franchise=f.id_franchise";
        $criteria->condition="(t.id_admin_role=1 or t.id_admin_role=2 or t.id_franchise=1) order by t.first_name asc";
        $assignToRows = Admin::model()->findAll($criteria);
        //echo '<pre>';print_r($assignToRows);exit;
        ...
        if (Yii::app()->request->getIsAjaxRequest() && $_POST[GpsDevice][update_on] != "" && (int) $_POST[Accountdeviceplanhistory][amount] != 0 && $_POST[Accountdeviceplanhistory][planName] != "") {
            ...
            $content = "<tbody ><tr>
                            <td>" . $adph->planName . "</td>
                                ...
                                <td>" . $adph->amount . "</td>
                                    <td>" . $received . "</td><td><textarea onblur='fnUpdateDetails(this)' rows='2' cols='10' name='planremark_".$adph->id."'  id='planremark_".$adph->id."' >".$adph->remark."</textarea></td>
    ...
    protected function grid($data, $row, $dataColumn) {
        switch ($dataColumn->name) {
            ...
            case 'creationTime':
                ...
                $timestamp = gmdate("M-y", $timestamp);
