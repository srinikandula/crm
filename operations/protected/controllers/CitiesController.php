...
class CitiesController extends Controller {
    ...
    {
        return $this->addActions(array('AutocompleteCity'));
    }
	...
	if (isset($_GET['term'])) {
            if($_GET['type']=='source'){
               $str=" source=1 and "; 
                ...
                $str=" destination=1 and "; 
            }    
            $qtxt ="SELECT concat(city,' - ',state) as name FROM {{city}} WHERE ".$str." city  LIKE :city";
            $command =Yii::app()->db->createCommand($qtxt);
            $command->bindValue(":city", '%'.$_GET['term'].'%', PDO::PARAM_STR);
