...
vehicleModel,lookingForLoad,lookingForLoadDate,fitnessExpire,insuranceExpire,rcNo,NPAvailable,NPExpire,insuranceAmount from Device where deviceID in (select deviceID from AttachedDevice) order by lastValidSpeedKPH desc";                        
						...
						$query = "select deviceID as truck_no,deviceID,ifnull(lastValidLatitude,0) as latitude,ifnull(lastValidLongitude,0) as longitude,concat(ceil(lastValidSpeedKPH*1.852),' ','Km/Hr') as speed, (ifnull(lastUpdateTime,0)+19800) as time_in_secs,FROM_UNIXTIME(ifnull(lastUpdateTime,0), '%d-%m-%y %H:%i') as date_time,concat(ceil(ifnull(lastOdometerKM,0)),' ','Km') as odometer,vehicleType,
vehicleModel,lookingForLoad,lookingForLoadDate,fitnessExpire,insuranceExpire,rcNo,NPAvailable,NPExpire,insuranceAmount from Device where ".$srchQry." accountID = '".$account_id."'  order by lastValidSpeedKPH desc";
                }
