...
class GPSapiV2Controller extends Controller {
    ...
    /*public function actionGetAlerts() { //need to upload image
        ...
        if (1) {
			if($type=='T'){
				$select="ga.*, DATE_FORMAT( ga.date_required,  '%d-%m-%Y %h:%i %p' )  as date_required";
			}else if($type=='C'){
				$select="ga.id_gps_alerts,ga.source,ga.destination,ga.id_truck_type_title,ga.price, DATE_FORMAT( ga.date_required,  '%d-%m-%Y %h:%i %p' )  as date_required";
			}else{
				$select="ga.*, DATE_FORMAT( ga.date_required,  '%d-%m-%Y %h:%i %p' )  as date_required";
			}
    ...
    public function actionSetCustomer() { //need to upload image
        ...
        */
        ...
        if ($mobile!="" && $fullname!="" && $address!="") {
			...
			$message=$fullname."(".$type." ".$mobile.") New Customer Sign Up";
			$this->sendMail(array("subject"=>$message,"message"=>$message));
...
vehicleModel,lookingForLoad,fitnessExpire,insuranceExpire,rcNo,NPAvailable,NPExpire,insuranceAmount FROM Device HAVING distance < ".$radius." ORDER BY distance";    
...
vehicleModel,lookingForLoad,fitnessExpire,insuranceExpire,rcNo,NPAvailable,NPExpire,insuranceAmount from Device where deviceID in (select deviceID from AttachedDevice) order by lastValidSpeedKPH desc";                        
...
vehicleModel,lookingForLoad,fitnessExpire,insuranceExpire,rcNo,NPAvailable,NPExpire,insuranceAmount from Device where accountID = '".$account_id."'  order by lastValidSpeedKPH desc";
                }
...
public function distance($lat1, $lon1, $lat2, $lon2, $unit) {
  ...
  if ($unit == "K") {
	...
	$data['lookingforload'] =(int)$_REQUEST['lookingforload'];
		//if (Yii::app()->request->isPostRequest) {
		if (1) {
                        GpsDeviceLoyalityPoints::model()->deleteAll('activity="lookingForLoad" and accountID="'.$data['accountid'].'" and date(dateCreated)="'.date('Y-m-d').'"');
                        $obj=new GpsDeviceLoyalityPoints;
                        $obj->accountID=$data['accountid'];
                        $obj->deviceID=$data['deviceid'];
                        $obj->activity='lookingForLoad';
                        $obj->dateCreated=$data['accountid'];
                        $obj->points=10;
                        $obj->status=0;
                        $obj->save();
