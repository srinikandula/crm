...
class ClassignController extends Controller
{   
        public $admins;
        public $adminOptions;
        public function actionUpdate($id)
	{
            ...
            if (Yii::app()->request->getIsAjaxRequest() && $aid && $id ) {
                Customerlead::model()->updateAll(array('id_admin_assigned'=>$aid),'id_customer='.$id);
                $model= new Customerleadassignment();
                $model->id_customer=$id;
                $model->id_admin_to=$aid;
                $model->id_admin_from=Yii::app()->user->id;
                $model->message="ReAssigned";
	...
	public function actionIndex()
	{
            ...
            $model=new Customer('customerAssignment');
            $model->unsetAttributes();  // clear any default values
            if(isset($_GET['Customer']))
                    $model->attributes=$_GET['Customer'];
            //exit;
            $this->render('index',array('model'=>$model,'dataSet'=>$model->customerAssignment()));
	}
        ...
        protected function grid($data, $row, $dataColumn) {
            switch ($dataColumn->name) {
                ...
                case 'id_admin_assigned';
                    //echo '<pre>';print_r($this->adminOptions);exit;
                    $select='<select name="select" onchange="fnchange(this,'.$data['id_customer'].')">';
