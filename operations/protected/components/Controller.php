...
class Controller extends CController {
    ...
    public function init() {
		if(in_array($this->uniqueid,array('dcapi'))){
			Yii::app()->db->enableProfiling=0; //disable profile block in the bottom
		}
			$this->getPermissions();
	  }
    ...
    protected function beforeAction($action) {
		...
		else if($action->id=='delete')
		{
			...
			$pageLastAccessed=$this->uniqueid."/".$action->id;
			if($actionName!="" && $_SESSION['pageLastAccessed']!=$pageLastAccessed){
				$_SESSION['pageLastAccessed']=$pageLastAccessed;
				$adminLogHistory=new AdminLogHistory;		
				$adminLogHistory->id_admin=Yii::app()->user->id;
				$adminLogHistory->page_accessed=$page_accessed;
				$adminLogHistory->page_url=$this->createUrl();//Yii::app()->getRequest()->getUrl();
